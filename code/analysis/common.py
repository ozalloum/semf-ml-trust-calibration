...
def chain_block_bootstrap(df: pd.DataFrame, y_col: str, a_col: str, b_col: str, n_boot: int = 5000, seed: int = 20260912) -> dict:
    """Isotope-chain block bootstrap with exact algebraic aggregation.

    This is mathematically identical to concatenating every resampled Z-chain and
    recomputing MAE, but avoids constructing thousands of temporary DataFrames.
    The RNG call remains one ``choice`` per replicate with the same seed and
    sorted chain labels, preserving the frozen bootstrap sampling sequence.
    """
    d=df[["Z",y_col,a_col,b_col]].dropna().copy()
    zs=np.array(sorted(d["Z"].unique()))
    if len(zs)==0:
        return {"n_chains":0}
    # Per-chain sufficient statistics for MAE. Repeating a chain in a block
    # bootstrap repeats both its absolute-error sum and its number of nuclei.
    counts=np.empty(len(zs),dtype=float)
    err_a=np.empty(len(zs),dtype=float)
    err_b=np.empty(len(zs),dtype=float)
    z_to_i={}
    for i,z in enumerate(zs):
        g=d[d["Z"]==z]
        y=g[y_col].to_numpy(float); aa=g[a_col].to_numpy(float); bb=g[b_col].to_numpy(float)
        counts[i]=len(g); err_a[i]=np.abs(aa-y).sum(); err_b[i]=np.abs(bb-y).sum(); z_to_i[z]=i
    rng=np.random.default_rng(seed)
    arr=np.empty((n_boot,2),dtype=float)
    for r in range(n_boot):
        sample_z=rng.choice(zs,size=len(zs),replace=True)
        ix=np.fromiter((z_to_i[z] for z in sample_z),dtype=int,count=len(sample_z))
        den=counts[ix].sum(); ea=err_a[ix].sum()/den; eb=err_b[ix].sum()/den
        arr[r,0]=ea-eb; arr[r,1]=ea/eb if eb>0 else np.nan
